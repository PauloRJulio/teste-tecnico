...
import type { TotalPorCategoria, RelatorioCategoriasResponse } from "../types/Relatorio";
import { relatorioService } from "../services/relatorioService";

export default function RelatorioCategoriasPage() {
    const [rows, setRows] = useState<TotalPorCategoria[]>([]);
    const [totals, setTotals] = useState<RelatorioCategoriasResponse | null>(null);
    ...
    async function carregarRelatorio() {
        const data = await relatorioService.obterTotaisPorCategoria();
        setRows(data.totaisPorCategorias || []);
        setTotals(data);
    }
    ...
    const columns: GridColDef<TotalPorCategoria>[] = [
        { field: "id", headerName: "ID", width: 90 },
        { field: "descricao", headerName: "Descrição", flex: 2, minWidth: 200 },
        { field: "finalidade", headerName: "Finalidade", flex: 1, minWidth: 120 },
        {
            field: "totalReceitas",
            headerName: "Receitas",
            flex: 1,
    ...
    return (
        <Box sx={{ width: "100%", mt: 2 }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h6">Transações Por Categoria</Typography>
